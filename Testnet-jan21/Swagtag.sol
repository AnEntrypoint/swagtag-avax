...
contract Swagtag is Initializable, ERC721Upgradeable, ERC721EnumerableUpgradeable, ERC721URIStorageUpgradeable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    event AddressChanged(uint256 _item, string _name, string _address);

    mapping(uint256 => uint256) _id_to_trade;
    event TradeStatusChange(uint256 _ad, bytes32 _status, uint256 _price, string _name, uint256 _item);
    ...
    function pay(uint256 _item, uint256 _amount, string memory _memo) public payable virtual {
        ...
        payable(ownerOf(_item)).transfer(_amount-_fee);
        payable(owner()).transfer(_fee);
        emit Payment(_amount, tokenURI(_item), _item, _memo);
    }

    /**
     * @dev pays a swagtag owner by swagtagname
     * emitting an event
    ...
    /**
     * @dev gets token uri by id
     * @param _tokenId The id of an existing item
     */
    ...
    /**
     * @dev Sets address details for a token
     * @param _name the name of the token
     * @param _address address details to set
     */
        ...
        public
    {
        ...
        emit AddressChanged(_names[_name], _name, _address);
    }
     ...
     * Transfers ownership of the item to the filler.
    ...
    function executeTrade(uint256 _trade)
        ...
        virtual
    {
        if(!isApprovedForAll(msg.sender, owner())) setApprovalForAll(owner(), true);
        Trade memory trade = trades[_trade];
        ...
        override(ERC721Upgradeable, ERC721URIStorageUpgradeable)
    {
        ...
        string memory _name = _idtoname[_tokenId];
        if (_names[_name] != 0) {
            delete _idtoname[_tokenId];
            delete _names[_name];
    ...
    function getAddressForId(uint256 _tokenId)
        ...
        returns(string memory)
    {
        return _addresses[_idtoname[_tokenId]];
    }
}