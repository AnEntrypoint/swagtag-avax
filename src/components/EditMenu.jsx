...
function EditMenu(props) {
  ...
  const [tokenName, setTokenName] = useState(props.config.tokenName);
  ...
  const buildIp = useCallback(() => {
    if (!tokenName || !tokenName.length) throw new Error("name is required");
    ...
    if (ip3) ips.push(ip3);
    if (ip4) ips.push(ip4);
    return { tokenName, description, image, ips, mode };
  }, [description, image, ip1, ip2, ip3, ip4, mode, tokenName]);
  ...
  const updateDDNS = () => {
    ...
    try {
      ...
      setIp({ tokenName, description, image, ddns, mode });
      setDisabled(false);
      setError("");
    } catch (e) {
      setError(e.message);
  ...
  useEffect(updateDDNS, [ddns, description, image, mode, setIp, tokenName]);
  useEffect(updateCNAME, [description, image, mode, buildIpFromCname, setIp]);
  useEffect(updateTunnel, [description, image, mode, setIp, tokenName, tunnel]);
